...
def covariance_Math_Science(students):
    mean_Math = sum([student["Math"] for student in students]) / len(students)
    mean_Science = sum([student["Science"] for student in students]) / len(students)
    covariance = sum([(student["Math"] - mean_Math) * (student["Science"] - mean_Science) for student in students]) / len(students)
    return covariance

covariance = covariance_Math_Science(students)
