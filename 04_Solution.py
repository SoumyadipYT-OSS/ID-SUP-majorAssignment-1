...
def mean_Computer_Science(students):
    return sum([student["Computer Science"] for student in students]) / len(students)


def median_Computer_Science(students):
    computer_science_marks = [student["Computer Science"] for student in students]
    computer_science_marks.sort()
    n = len(computer_science_marks)
    if n % 2 == 0:
        return (computer_science_marks[n//2 - 1] + computer_science_marks[n//2]) / 2
    return computer_science_marks[n//2] + 1


def mode_Computer_Science(students):
    computer_science_marks = [student["Computer Science"] for student in students]
    return max(set(computer_science_marks), key = computer_science_marks.count)



mean = mean_Computer_Science(students)
median = median_Computer_Science(students)
mode = mode_Computer_Science(students)


print("Mean of Computer Science marks: ", mean)
print("Median of Computer Science marks: ", median)
print("Mode of Computer Science marks: ", mode)